...
- **NO exponer (irreducible → no replicable):** el mecanismo del método — la secuencia neural, el cómo real de la transformación. Eso se queda en los videos irreducibles (§7) y en lo presencial. **El KB no debe volverse todo público:** la síntesis profunda del método es el activo durable, y publicarla en formato parseable sería regalarle a cualquier IA justo el foso. Es la misma lógica del video irreducible, aplicada a la capa pública.
...
- **Dejar que el KB alimente esa capa** — pero curando qué es público (identidad/resultados) y qué se queda privado (mecanismo).
...
- **2026-06-03** (refinamiento) — Recomendación AEO evaluada contra el radar a fondo (lectura de fuentes crudas). Integrado el **guardrail de la paradoja de la legibilidad** (Nate Jones): legibilidad parcial — exponer identidad + resultados, NO el mecanismo del método; **el KB no debe volverse todo público** (la síntesis profunda es el foso). Es la misma lógica del video irreducible (§7) aplicada a la capa pública. Ajustado el matiz de schema markup (prioridad baja, no cero — útil para descubrimiento local, después de la página canónica).
